...
export const Route = createFileRoute("/hackathons/$id/")({
  head: () => ({
    meta: [
      { title: "Hackathon dashboard — Submission Verifier" },
      {
        name: "description",
        content:
          "Scan every submission with relevance, verified claims and open issues in one dense table.",
      },
      { property: "og:title", content: "Hackathon dashboard" },
      {
...
function Dashboard() {
  ...
  return (
    <AppShell>
      <Crumbs items={[{ label: "Hackathons", to: "/" }, { label: hackathon.name }]} />
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 font-mono text-xs text-muted-foreground">
            {hackathon.submissionStart.slice(0, 10)} →{" "}
            {hackathon.submissionEnd.slice(0, 10)}
          </p>
        </div>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3 rounded-md border border-border bg-card px-3 py-2.5">
        ...
        <div className="flex min-w-52 items-center gap-3">
          ...
          <Slider
            ...
            className="w-32"
          />
          <span className="w-7 font-mono text-xs tabular-nums">
            {minRelevance}
          </span>
        </div>
      ...
      <div className="mt-3 overflow-hidden rounded-md border border-border bg-card">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((s, i) => (
              <Row key={s.id} rank={i + 1} submission={s} hackathonId={id} onOpen={() =>
                navigate({
...
function Row({
  ...
  return (
    ...
    >
      ...
      <td className="px-3 py-2">
        <ClaimsMeter
          verified={s.claimsVerifiedCount}
          total={s.claimsTotalCount}
        />
      </td>
      <td className="px-3 py-2">
